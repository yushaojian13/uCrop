 ...
 */
open class CropImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : TransformImageView(context, attrs, defStyle) {
    ...
    fun cropAndSaveImage(compressFormat: CompressFormat, compressQuality: Int,
        ...
        BitmapCropTask(context, viewBitmap, imageState, cropParameters, cropCallback).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR)
    }
     ...
     *
     * @param - aspect ratio for image crop (e.g. 1.77(7) for 16:9)
     */
    ...
    private fun calculateImageIndents(): FloatArray {
        ...
        indents[0] = if (deltaLeft > 0f) deltaLeft else 0f
        indents[1] = if (deltaTop > 0f) deltaTop else 0f
        indents[2] = if (deltaRight < 0f) deltaRight else 0f
        indents[3] = if (deltaBottom < 0f) deltaBottom else 0f
    ...
    protected fun zoomImageToPosition(scale: Float, centerX: Float, centerY: Float, durationMs: Long) {
        val scaleTemp = min(scale, maxScale)

        val oldScale = currentScale
        val deltaScale = scaleTemp - oldScale
    ...
    fun processStyledAttributes(a: TypedArray) {
        val targetAspectRatioX = abs(a.getFloat(R.styleable.ucrop_UCropView_ucrop_aspect_ratio_x, DEFAULT_ASPECT_RATIO))
        val targetAspectRatioY = abs(a.getFloat(R.styleable.ucrop_UCropView_ucrop_aspect_ratio_y, DEFAULT_ASPECT_RATIO))
