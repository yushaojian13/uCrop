 ...
 */
class OverlayView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null, defStyle: Int = 0) : View(context, attrs, defStyle) {
    ...
    private fun setupCropBounds() {
        ...
        if (height > mThisHeight) {
            ...
            cropViewRect[paddingLeft + halfDiff.toFloat(), paddingTop.toFloat(), paddingLeft + width + halfDiff.toFloat()] = paddingTop + mThisHeight.toFloat()
        } else {
            val halfDiff = (mThisHeight - height) / 2
            cropViewRect[paddingLeft.toFloat(), paddingTop + halfDiff.toFloat(), paddingLeft + mThisWidth.toFloat()] = paddingTop + height + halfDiff.toFloat()
        }
    ...
    private fun updateGridPoints() {
        ...
        mGridPoints = null
        mCircularPath.reset()
        mCircularPath.addCircle(cropViewRect.centerX(), cropViewRect.centerY(), min(cropViewRect.width(), cropViewRect.height()) / 2f, Path.Direction.CW)
    }
