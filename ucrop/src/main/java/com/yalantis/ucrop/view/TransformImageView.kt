 ...
 */
open class TransformImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : AppCompatImageView(context, attrs, defStyle) {

    @JvmField protected val mCurrentImageCorners = FloatArray(RECT_CORNER_POINTS_COORDS)
    @JvmField protected val mCurrentImageCenter = FloatArray(RECT_CENTER_POINT_COORDS)

    private val mMatrixValues = FloatArray(MATRIX_VALUES_COUNT)

    @JvmField protected var mCurrentImageMatrix = Matrix()
    @JvmField protected var mThisWidth = 0
    @JvmField protected var mThisHeight = 0

    @JvmField protected var mTransformImageListener: TransformImageListener? = null
    ...
    @JvmField protected var mBitmapLaidOut = false
     ...
     *
     * @param - max size for both width and height of bitmap that will be used in the view.
     */
    ...
    fun setImageUri(imageUri: Uri, outputUri: Uri?) {
        ...
        decodeBitmapInBackground(context, imageUri, outputUri, maxBitmapSize, maxBitmapSize,
